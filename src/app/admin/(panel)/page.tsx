...
function StatCard(props: { label: string; value: string; href?: string }) {
  const inner = (
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-2 text-2xl font-bold text-[#B91C1C]">{props.value}</p>
    </div>
  );

  return props.href ? (
    <Link href={props.href} className="block hover:border-[#B91C1C]">
      {inner}
    </Link>
  ) : (
    inner
    ...
    <div className="space-y-6">
      <section className="rounded-lg bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-xl font-bold text-[#B91C1C]">Dashboard</h2>
      ...
      <section className="rounded-lg bg-white p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-[#B91C1C]">Status Sistem</h3>
