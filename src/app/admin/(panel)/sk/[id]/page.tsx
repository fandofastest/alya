...
export default async function AdminSkEditPage(props: { params: Promise<{ id: string }> }) {
  const { locale, t } = await getI18n();
  const { id } = await props.params;

  await connectDb();
  const item = await SimpleDocumentModel.findById(id).lean();
  if (!item || item.type !== "sk") return notFound();

  return (
    <div className="space-y-6">
      <section className="rounded-lg bg-white p-6 shadow-sm">
        <h2 className="text-xl font-bold text-[#B91C1C]">{t.common.edit} {t.documents.sk.singular}</h2>
