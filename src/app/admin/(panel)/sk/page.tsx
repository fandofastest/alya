...
export default async function AdminSkPage() {
  const { locale, t } = await getI18n();
  await connectDb();
  const items = await SimpleDocumentModel.find({ type: "sk" }).sort({ updatedAt: -1 }).lean();

  return (
    <div className="space-y-6">
      <section className="flex items-center justify-between rounded-lg bg-white p-6 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-[#B91C1C]">{t.documents.sk.plural}</h2>
                ...
                items.map((item) => (
                  <tr key={item._id.toString()} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-right space-x-4">
                      <Link
                        href={`/admin/sk/${item._id.toString()}`}
