...
export async function GET() {
  ...
  await connectDb();
  const users = await UserModel.find().sort({ createdAt: -1 }).select("_id email nama isActive createdAt").lean();
...
export async function POST(request: Request) {
  ...
  const exists = await UserModel.findOne({ email: email.toLowerCase() }).select("_id").lean();
  if (exists) return jsonError("Email sudah terdaftar.", 400);
  ...
  return NextResponse.json(
    { message: "User berhasil dibuat.", data: { id: created._id, email: created.email, nama: created.nama, isActive: created.isActive } },
    { status: 201 }
  );
}
