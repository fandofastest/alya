...
export function AdminLoginForm(props: { locale: Locale }) {
  ...
  return (
    <div className="mx-auto w-full max-w-md rounded-lg bg-white p-6 shadow-sm">
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500">{t.admin.panel}</p>
        <h1 className="text-2xl font-black text-[#B91C1C]">{t.login.adminTitle}</h1>
        <p className="text-sm text-slate-600">{t.login.adminSubtitle}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-5 space-y-4">
        <label className="block space-y-1 text-sm">
          <span className="font-semibold text-slate-700">{t.login.email}</span>
          <input
            type="email"
            value={email}
